...
class IntacctClient:
    ...
    def read_by_query(
        ...
        Note:
            `readByQuery` (dtdversion 3.0) does NOT support server-side sort.
            Records come back in Intacct storage order (roughly creation order)
            and `pagesize` truncates against that order. For "most recent N"
            queries, use a narrow `WHENCREATED >= ...` filter and expand the
            window if the page is underfilled — see `get_recent_transactions`.
